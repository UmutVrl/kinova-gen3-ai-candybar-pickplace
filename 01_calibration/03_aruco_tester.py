...
#                                                                                       #
#    Testing camera calibration with aruco markers                                      #
#                                                                                       #
#    written by: U. Vural                                                               #
#                                                                                       #
#                                                                                       #
        ...
        while cv2.waitKey(1) != 27:
            try:
                ...
                if ids is not None:
                    ...
                    corners1 = find_marker_by_id(corners, ids, MARKER_ID_1)
                    corners2 = find_marker_by_id(corners, ids, MARKER_ID_2)
                    ...
                    if corners1 is not None and corners2 is not None:
                        ...
                        idx2 = [i for i, id in enumerate(ids) if id[0] == MARKER_ID_2][0]
                        tvec1 = tvecs[idx1][0]
                        tvec2 = tvecs[idx2][0]

                        # Calculate and display distance between the two specified markers
                        distance = calculate_distance(tvec1, tvec2)
                        cv2.putText(frame,
                                    f"Distance between markers {MARKER_ID_1} and {MARKER_ID_2}: {distance:.2f} cm",
                                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

                        # tvecs[idx][0] is the translation vector for marker at index idx
                        distance_to_camera = np.linalg.norm(tvecs[idx1])
                        cv2.putText(frame,
                                    f"Distance from camera to marker {MARKER_ID_1} : {distance_to_camera:.2f} cm",
                                    (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)


                        # Draw circles at the centers of the two markers
                        center1 = tuple(map(int, corners1.mean(axis=0)))
                        center2 = tuple(map(int, corners2.mean(axis=0)))
                        cv2.circle(frame, center1, 5, (0, 255, 0), -1)
