...
def print_extrinsic_parameters(extrinsics):
    ...
    print("[{0: .6f} {1: .6f} {2: .6f}".format(extrinsics.rotation.row1.column1,
                                               extrinsics.rotation.row1.column2, extrinsics.rotation.row1.column3))
    print(" {0: .6f} {1: .6f} {2: .6f}".format(extrinsics.rotation.row2.column1,
                                               extrinsics.rotation.row2.column2, extrinsics.rotation.row2.column3))
    print(" {0: .6f} {1: .6f} {2: .6f}]".format(extrinsics.rotation.row3.column1,
                                                extrinsics.rotation.row3.column2, extrinsics.rotation.row3.column3))
    print("Translation vector: [{0:.6f} {1:.6f} {2:.6f}]".format(extrinsics.translation.t_x,
...
def set_custom_extrinsics(vision_config, vision_device_id):
    extrinsics_new = VisionConfig_pb2.ExtrinsicParameters()

    print("\n\n** Setting custom extrinsic parameters based on physical measurements **")
    ...
    # Set rotation matrix (assuming no rotation, set to identity matrix)
    extrinsics_new.rotation.row1.column1 = 1.0
    extrinsics_new.rotation.row1.column2 = 0.0
    extrinsics_new.rotation.row1.column3 = 0.0
    extrinsics_new.rotation.row2.column1 = 0.0
    extrinsics_new.rotation.row2.column2 = 1.0
    extrinsics_new.rotation.row2.column3 = 0.0
    extrinsics_new.rotation.row3.column1 = 0.0
    extrinsics_new.rotation.row3.column2 = 0.0
    extrinsics_new.rotation.row3.column3 = 1.0
    ...
    extrinsics_new.translation.t_x = 0.05638  # 5.638 cm
    extrinsics_new.translation.t_y = 0.0  # 0 cm
    extrinsics_new.translation.t_z = -0.12365  # -12.365 cm

    # Apply the new extrinsic parameters
    ...
    return extrinsics_new  # Return the new extrinsics in case you need them later
